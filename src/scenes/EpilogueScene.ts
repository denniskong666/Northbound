...
export class EpilogueScene extends BaseScene {
  ...
  protected getChapterTint(_ch: ChapterId): { color: number; alpha: number } {
    switch (GameState.inst.computeEnding()) {
      ...
      case 'betrayal':      return { color: 0x4a1a1a, alpha: 0.30 };  // 暗红背叛（同故土背景但氛围暗沉）
  ...
  protected spawnContent(): void {
    ...
    switch (ending) {
      ...
      case 'betrayal':      this.spawnBetrayalItems(); break;
    }
  ...
  private spawnAtmosphere(ending: EndingType | null): void {
    ...
    // 背叛结局：复用故土黄昏背景，但叠加暗红色调
    if (ending === 'betrayal') {
      this.spawnHomeScene();
      // 叠加暗红渐变层，强化背叛氛围
      const overlay = this.add.graphics().setDepth(-1);
      overlay.fillGradientStyle(0x2a0a0a, 0x2a0a0a, 0x4a1a1a, 0x4a1a1a, 0.45);
  ...
  private showEndingTitle(ending: EndingType | null): void {
    ...
    const subtitle: Record<EndingType, string> = {
      ...
      betrayal:      L('你头也不回地走了，把所有人的挽留抛在身后。', 'You walked away without looking back, leaving every plea behind.'),
  ...
  // ================================================================
  // 隐藏结局【背弃故乡】betrayal（tile坐标同 return_home，但物品提示全部改为背叛语言）
  // 触发：第四章车库 Elias 选 B → ending = 'betrayal'
  // 背景：与 return_home 相同的老街黄昏氛围 + 暗红叠加
  // ================================================================
  private spawnBetrayalItems(): void {
    this.showChoiceSummary();

    // 第一排（y=3）：核心物品，间距5格（同 return_home 坐标）
    this.addPoi(4, 3, L('被丢下的画', 'Discarded Painting'), {
      onInteract: () => this.showSpeech(this.getBetrayalMayaDesc())
    });
    this.addPoi(9, 3, L('被扔下的手工', 'Abandoned Craft'), {
      onInteract: () => this.showSpeech(this.getBetrayalNoahDesc())
    });
    this.addPoi(14, 3, L('背对的旧物', 'Turned-Away Keepsake'), {
      onInteract: () => this.showSpeech(this.getBetrayalLeoDesc())
    });
    this.addPoi(19, 3, L('偷拿的材料', 'Stolen Papers'), {
      onInteract: () => this.showSpeech(this.getBetrayalMaterialDesc())
    });
    // 第二排（y=5）：窗外
    this.addPoi(12, 5, L('不敢回望的窗', 'Window Dare Not Look Back'), {
      onInteract: () => this.showSpeech(this.getBetrayalWindowDesc())
    });
  }
  ...
  // ================================================================
  // 隐藏结局【背弃故乡】物品描述生成器
  // 全部使用背叛相关语言，与 return_home 的温暖语言形成对比
  // ================================================================

  // 被丢下的画（Maya）
  private getBetrayalMayaDesc(): string {
    const bond = GameState.inst.bond.maya;
    if (bond >= 10) {
      return L('Maya 把这幅画留在桌上等你来取——你以为自己只是迟到，却再也没回去过。\n你曾答应她看画展，又曾答应留下来——两件事你都背叛了。', 'Maya left this painting on the table, waiting for you to pick it up — you thought you were just late, but you never went back.\nYou once promised to attend her show, and once promised to stay — you betrayed both.');
    }
    return L('Maya 的画被你丢在桌上，落了灰。\n你曾答应她看画展，最终却趁她不备，悄悄跟着 Elias 走了。\n「我没想到你会这样。」她后来对别人说。', "Maya's painting lies discarded on the table, covered in dust.\nYou once promised to go to her show — but in the end, while she wasn't looking, you slipped away with Elias.\n\"I never thought you'd do this,\" she later said to someone.");
  }

  // 被扔下的手工（Noah）
  private getBetrayalNoahDesc(): string {
    const bond = GameState.inst.bond.noah;
    if (bond >= 10) {
      return L('Noah 把最得意的手工送给你作信物，你却走得匆忙，把它留在了他的窗台。\n你曾鼓励他追求手工艺，自己却在他最需要支持时背叛了这份共同的执着。', 'Noah gave you his proudest handmade piece as a token, but you left in such a hurry that you left it on his windowsill.\nYou once encouraged him to pursue his craft, yet betrayed that shared conviction when he needed support most.');
    }
    return L('Noah 的手工被你扔在角落，沾了灰。\n他曾以为你会和他一起留下来做手工——你连一句告别都没说就走。', "Noah's craft lies tossed in a corner, gathering dust.\nHe thought you would stay and do craftwork with him — you left without a single word of goodbye.");
  }

  // 背对的旧物（Leo）
  private getBetrayalLeoDesc(): string {
    const bond = GameState.inst.bond.leo;
    if (bond >= 10) {
      return L('Leo 珍视了一辈子的老街旧物，你假装看不见，背过身去。\n他曾以为你和他是同一种人——你证明他错了。', 'The Old Street keepsake Leo treasured his whole life — you pretended not to see it and turned your back.\nHe once thought you were the same kind of person as him — you proved him wrong.');
    }
    return L('Leo 的老街旧物被你一脚踢到墙角。\n他曾对你说过他舍不得这里——你头也不回地走了，连一句回应都没给他。', "Leo's Old Street keepsake was kicked into the corner by you.\nHe once told you he couldn't bear to leave this place — you walked away without looking back, without a single reply.");
  }

  // 偷拿的材料（与 return_home 的"封存材料"形成对比）
  private getBetrayalMaterialDesc(): string {
    return L('你没有封存通行材料——你趁众人不备，抓起 Elias 加急办好的全套手续，偷偷跑掉了。\n这是你背叛故土最直接的证据。\n所有名字都在背面，唯独你的签名格外刺眼。', 'You did not seal the travel papers — while no one was watching, you grabbed the full set Elias had expedited and slipped away.\nThis is the most direct evidence of your betrayal of your hometown.\nAll the names are on the back, but yours stands out, jarringly sharp.');
  }

  // 不敢回望的窗
  private getBetrayalWindowDesc(): string {
    return L('窗外是老街的灯火。你头也不回地走了，没敢看最后一眼。\n每一盏灯都是你曾熟悉的光——你把它们全部抛弃在身后。\n「我做了正确的事。」你对自己说，但声音很轻。', 'Outside the window are the lights of the Old Street. You walked away without looking back, not daring to take one last glance.\nEvery lamp is a light you once knew — you abandoned them all behind you.\n"I did the right thing," you tell yourself, but your voice is very quiet.');
  }
