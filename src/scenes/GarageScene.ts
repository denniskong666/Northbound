...
import { NpcPlacement, NpcId } from '../data/NpcDefs';
...
import { CH4_GARAGE_ELIAS_DIALOGUE } from '../data/Dialogues';
...
export class GarageScene extends BaseScene {
  ...
  // 第四章隐藏支线：玩家偏向留下时，车库 Elias 触发新对话（不重复第一章）
  // 不偏向留下时，使用父类默认 DIALOGUES['elias']
  protected getChapterStoryDialogue(npcId: NpcId, ch: ChapterId): DialogueData {
    if (ch === 'ch4' && npcId === 'elias' && GameState.inst.leansStaying()) {
      return CH4_GARAGE_ELIAS_DIALOGUE;
    }
    return super.getChapterStoryDialogue(npcId, ch);
  }

  // 第四章选 B 反悔北上后：Elias 对话结束 → 直接进入终章
  protected onNpcDialogueComplete(_npcId: NpcId): void {
    if (GameState.inst.chapter === 'ch4' &&
        _npcId === 'elias' &&
        GameState.inst.computeEnding() === 'betrayal') {
      // 触发了背弃故乡结局：直接跳转 EpilogueScene
