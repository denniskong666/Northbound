...
export class OldDistrictScene extends BaseScene {
  ...
  protected spawnContent(): void {
        ...
        onInteract: () => {
          ...
          const startMain = () => {
            this.dialogueSystem.start(CH3_PASS_DIALOGUE, () => {
              ...
              // 关键修复：对话结束后立即刷新可选支线 POI，无需场景切换
              this.refreshSideQuestPois();
            });
          };
          if (opening) {
    ...
    // —— 可选支线（ch3_maya_help / ch4_memory_walk）——
    // 统一通过 refreshSideQuestPois 生成，便于在对话完成回调里立即调用
    this.refreshSideQuestPois();
  ...
  // —— 统一刷新可选支线 POI ——
  // 关键修复：在 ch3_pass / ch4_organize 对话完成回调中立即调用，
  // 让 ch3_maya_help / ch4_memory_walk 的 POI 在对话结束后立刻生成，无需场景切换
  // 方法是幂等的：spawnCh3MayaHelpPois / spawnCh4MemoryWalkPois 内部已用 flag 过滤已收集点位
  protected refreshSideQuestPois(): void {
    const ch = GameState.inst.chapter;
    ...
    if (ch === 'ch3' && TaskSystem.inst.isUnlocked('ch3_maya_help') && !TaskSystem.inst.isDone('ch3_maya_help')) {
      ...
      if (fullA) {
        ...
        } else if (!GameState.inst.hasFlag('ch3_maya_help_locked_toast_shown')) {
          // 锁定提示只显示一次，避免重复打扰
          this.showToast(L('画展支线：需要与玛雅有更深的羁绊', "Exhibit sidequest: needs a deeper bond with Maya"));
          GameState.inst.applyEffects({ flag: 'ch3_maya_help_locked_toast_shown' });
        }
      } else {
        this.spawnCh3MayaHelpPois();
      }
